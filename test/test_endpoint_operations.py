...
def setup_class(cls, opts):
    ...
    ch.setFormatter(
        logging.Formatter(f"[%(asctime)s] [{cls.__name__}] [%(levelname)s] %(message)s")
    )
    ...
    try:
        ...
        cls._class_init_error = True
        cls.logger.debug(
            f"Failed to authenticate as rodsadmin [{cls.rodsadmin_username}]."
        )
        return
    ...
    try:
        cls.api.users_groups.create_user(
            cls.rodsuser_username, cls.zone_name, "rodsuser"
        )
        ...
        cls._class_init_error = True
        cls.logger.debug(
            f"Failed to authenticate as rodsuser [{cls.rodsuser_username}]."
        )
        return
...
class collectionsTests(unittest.TestCase):
    ...
    def testCreate(self):
        ...
        response = self.api.collections.create(f"{self.zone_name}/home/new")
        self.assertEqual(
            "{'irods_response': {'status_code': -358000, 'status_message': 'path does not exist: OBJ_PATH_DOES_NOT_EXIST'}}",
            str(response["data"]),
        )
    ...
    def testRemove(self):
        ...
        self.assertEqual(
            "{'irods_response': {'status_code': 0}}", str(response["data"])
        )
        # test invalid paths
        response = self.api.collections.stat(
            f"/{self.zone_name}/home/tensaitekinaaidorusama"
        )
        self.assertEqual(
            "{'irods_response': {'status_code': -170000}}", str(response["data"])
        )
        response = self.api.collections.stat(
            f"/{self.zone_name}/home/aremonainainaikoremonainainai"
        )
        self.assertEqual(
            "{'irods_response': {'status_code': -170000}}", str(response["data"])
        )
        response = self.api.collections.stat(
            f"/{self.zone_name}/home/binglebangledingledangle"
        )
        self.assertEqual(
            "{'irods_response': {'status_code': -170000}}", str(response["data"])
        )
        ...
        self.assertEqual(
            "{'irods_response': {'status_code': -79000, 'status_message': 'cannot remove non-empty collection: SYS_COLLECTION_NOT_EMPTY'}}",
            str(response["data"]),
        )
    ...
    def testList(self):
        ...
        response = self.api.collections.list(
            f"/{self.zone_name}/home/{self.rodsadmin_username}"
        )
        ...
        # test with recursion
        response = self.api.collections.list(
            f"/{self.zone_name}/home/{self.rodsadmin_username}", 1
        )
        self.assertEqual(
            f"/{self.zone_name}/home/{self.rodsadmin_username}/albania",
            str(response["data"]["entries"][0]),
    ...
    def testSetPermission(self):
        ...
        # test param checking
        self.assertRaises(
            TypeError, self.api.collections.set_permission, 0, "jeb", "read", 0
        )
        self.assertRaises(
            TypeError,
    ...
    def testSetInheritance(self):
        ...
        response = self.api.collections.set_inheritance(
            f"/{self.zone_name}/home/{self.rodsadmin_username}", 1
        )
        self.assertEqual(
            "{'irods_response': {'status_code': 0}}", str(response["data"])
        ...
        response = self.api.collections.set_inheritance(
            f"/{self.zone_name}/home/{self.rodsadmin_username}", 0
        )
        self.assertEqual(
            "{'irods_response': {'status_code': 0}}", str(response["data"])
        )
    ...
    def testModifyPermissions(self):
        ...
        # test param checking
        self.assertRaises(
            TypeError, self.api.collections.modify_permissions, 0, ops_permissions, 0
        )
        self.assertRaises(
            TypeError,
        ...
        self.api.setToken(self.rodsadmin_bearer_token)
        response = self.api.collections.modify_permissions(
            f"/{self.zone_name}/home/modPerms", ops_permissions
        )
        self.assertEqual(response["data"]["irods_response"]["status_code"], 0)
        ...
        self.api.setToken(self.rodsadmin_bearer_token)
        response = self.api.collections.modify_permissions(
            f"/{self.zone_name}/home/modPerms", ops_permissions_null
        )
        self.assertEqual(response["data"]["irods_response"]["status_code"], 0)
    ...
    def testModifyMetadata(self):
        ...
        ops_metadata_remove = [
            {"operation": "remove", "attribute": "eyeballs", "value": "itchy"}
        ]

        # test param checking
        self.assertRaises(
            TypeError, self.api.collections.modify_metadata, 0, ops_metadata, 0
        )
        self.assertRaises(
            TypeError,
    ...
    def testRename(self):
        ...
        self.assertRaises(
            TypeError, self.api.collections.rename, 0, f"/{self.zone_name}/home/pods"
        )
...
class dataObjectsTests(unittest.TestCase):
    ...
    def testCommonOperations(self):
        ...
        try:
            ...
            r = self.api.resources.create(
                "resource", "unixfilesystem", self.host, "/tmp/resource", ""
            )
            ...
            r = self.api.data_objects.write(
                ...
                f"/{self.zone_name}/home/{self.rodsuser_username}/file.txt",
                "resource",
            )
            self.assertEqual(r["data"]["irods_response"]["status_code"], 0)
            ...
            r = self.api.data_objects.trim(
                f"/{self.zone_name}/home/{self.rodsuser_username}/file.txt", 0
            )
            self.assertEqual(r["data"]["irods_response"]["status_code"], 0)

            # Rename the data object
            r = self.api.data_objects.rename(
                f"/{self.zone_name}/home/{self.rodsuser_username}/file.txt",
                f"/{self.zone_name}/home/{self.rodsuser_username}/newfile.txt",
            )
            self.assertEqual(r["data"]["irods_response"]["status_code"], 0)

            # Copy the data object
            r = self.api.data_objects.copy(
                f"/{self.zone_name}/home/{self.rodsuser_username}/newfile.txt",
                f"/{self.zone_name}/home/{self.rodsuser_username}/anotherfile.txt",
            )
            self.assertEqual(r["data"]["irods_response"]["status_code"], 0)
            ...
            r = self.api.data_objects.remove(
                f"/{self.zone_name}/home/{self.rodsuser_username}/anotherfile.txt", 0, 1
            )
            self.assertEqual(r["data"]["irods_response"]["status_code"], 0)

            r = self.api.data_objects.remove(
                f"/{self.zone_name}/home/{self.rodsuser_username}/newfile.txt", 0, 1
            )
            self.assertEqual(r["data"]["irods_response"]["status_code"], 0)
    ...
    def testChecksums(self):
        ...
        # Create a unixfilesystem resource.
        r = self.api.resources.create(
            "newresource", "unixfilesystem", self.host, "/tmp/newresource", ""
        )
        self.assertEqual(r["data"]["irods_response"]["status_code"], 0)
        ...
        try:
            ...
            r = self.api.data_objects.verify_checksum(
                f"/{self.zone_name}/home/{self.rodsadmin_username}/file.txt"
            )
            self.assertEqual(r["data"]["irods_response"]["status_code"], 0)
        finally:
            # Remove the data objects
            r = self.api.data_objects.remove(
                f"/{self.zone_name}/home/{self.rodsadmin_username}/file.txt", 0, 1
            )
            self.assertEqual(r["data"]["irods_response"]["status_code"], 0)
    ...
    def testTouch(self):
        ...
        r = self.api.data_objects.touch(
            f"/{self.zone_name}/home/{self.rodsadmin_username}/new.txt", 1
        )
        self.assertEqual(r["data"]["irods_response"]["status_code"], 0)

        # Show that the object has not been created
        r = self.api.data_objects.stat(
            f"/{self.zone_name}/home/{self.rodsadmin_username}/new.txt"
        )
        self.assertEqual(r["data"]["irods_response"]["status_code"], -171000)

        # Test touching non existant object without no_create
        r = self.api.data_objects.touch(
            f"/{self.zone_name}/home/{self.rodsadmin_username}/new.txt", 0
        )
        self.assertEqual(r["data"]["irods_response"]["status_code"], 0)
    ...
    def testRegister(self):
        ...
        r = self.api.data_objects.stat(
            f"/{self.zone_name}/home/{self.rodsadmin_username}/register-demo.txt"
        )
        self.assertEqual(r["data"]["irods_response"]["status_code"], -171000)
        ...
        try:
            ...
            r = self.api.data_objects.remove(
                f"/{self.zone_name}/home/{self.rodsadmin_username}/register-demo.txt", 1
            )
            self.assertEqual(r["data"]["irods_response"]["status_code"], 0)
    ...
    def testParallelWrite(self):
        self.api.setToken(self.rodsadmin_bearer_token)
        self.api.data_objects.remove(
            f"/{self.zone_name}/home/{self.rodsadmin_username}/parallel-write.txt", 0, 1
        )
...
class resourcesTests(unittest.TestCase):
    ...
    def testCommonOperations(self):
        ...
        for resc_name in [resc_ufs0, resc_ufs1]:
            with self.subTest(
                f"Create and attach resource [{resc_name}] to [{resc_repl}]"
            ):
                vault_path = f"/tmp/{resc_name}_vault"

                # Create a unixfilesystem resource.
                r = self.api.resources.create(
                    resc_name, "unixfilesystem", self.host, vault_path, ""
                )
                self.assertEqual(r["data"]["irods_response"]["status_code"], 0)
        ...
        r = self.api.data_objects.write(
            "These are the bytes to be written", data_object, resc_repl, 0
        )
        self.assertEqual(r["data"]["irods_response"]["status_code"], 0)

        # Show there are two replicas under the replication resource hierarchy.
        r = self.api.queries.execute_genquery(
            f"select DATA_NAME, RESC_NAME where DATA_NAME = '{os.path.basename(data_object)}'"
        )
        self.assertEqual(r["data"]["irods_response"]["status_code"], 0)
        ...
        for resc_name in [resc_ufs0, resc_ufs1]:
            with self.subTest(
                f"Detach and remove resource [{resc_name}] from [{resc_repl}]"
            ):
    ...
    def testModifyMetadata(self):
        ...
        # Create a unixfilesystem resource.
        r = self.api.resources.create(
            "metadata_demo", "unixfilesystem", self.host, "/tmp/metadata_demo_vault", ""
        )
        self.assertEqual(r["data"]["irods_response"]["status_code"], 0)

        operations = [
            {"operation": "add", "attribute": "a1", "value": "v1", "units": "u1"}
        ]
        ...
        r = self.api.queries.execute_genquery(
            "select RESC_NAME where META_RESC_ATTR_NAME = 'a1' and META_RESC_ATTR_VALUE = 'v1' and META_RESC_ATTR_UNITS = 'u1'"
        )
        self.assertEqual(r["data"]["irods_response"]["status_code"], 0)
        ...
        operations = [
            {"operation": "remove", "attribute": "a1", "value": "v1", "units": "u1"}
        ]
...
class rulesTests(unittest.TestCase):
    ...
    def testRemoveDelayRule(self):
        ...
        r = self.api.rules.execute(
            f'delay("<INST_NAME>{rep_instance}</INST_NAME><PLUSET>1h</PLUSET>") {{ writeLine("serverLog", "iRODS HTTP API"); }}',
            rep_instance,
        )
...
class ticketsTests(unittest.TestCase):
    ...
    def testCreateAndRemove(self):
        ...
        # for this.
        r = self.api.queries.execute_genquery(
            "select TICKET_STRING, TICKET_TYPE, TICKET_COLL_NAME, TICKET_USES_LIMIT, TICKET_ALLOWED_USER_NAME, TICKET_ALLOWED_GROUP_NAME, TICKET_ALLOWED_HOST"
        )
...
class userTests(unittest.TestCase):
    ...
    def test_create_stat_and_remove_rodsuser(self):
        ...
        self.assertEqual(
            stat_info["local_unique_name"], f"{new_username}#{self.zone_name}"
        )
    ...
    def test_set_password(self):
        ...
        # Set a new password
        r = self.api.users_groups.set_password(
            new_username, self.zone_name, new_password
        )
        self.assertEqual(r["status_code"], 200)
    ...
    def test_add_remove_user_to_and_from_group(self):
        ...
        r = self.api.users_groups.is_member_of_group(
            new_group, new_username, self.zone_name
        )
        self.assertEqual(r["status_code"], 200)
        result = r["data"]
        ...
        r = self.api.users_groups.remove_from_group(
            new_username, self.zone_name, new_group
        )
    ...
    def test_only_a_rodsadmin_can_change_the_type_of_a_user(self):
        ...
        r = self.api.users_groups.set_user_type(
            new_username, self.zone_name, new_user_type
        )
        self.assertEqual(r["status_code"], 200)
        self.assertEqual(r["data"]["irods_response"]["status_code"], 0)
        ...
        self.api.setToken(self.rodsuser_bearer_token)
        r = self.api.users_groups.set_user_type(
            new_user_type, self.zone_name, new_user_type
        )
        self.assertEqual(r["status_code"], 200)
    ...
    def test_listing_all_users_in_zone(self):
        ...
        self.assertEqual(result["irods_response"]["status_code"], 0)
        self.assertIn(
            {"name": self.rodsadmin_username, "zone": self.zone_name}, result["users"]
        )
        self.assertIn(
            {"name": self.rodsuser_username, "zone": self.zone_name}, result["users"]
        )
    ...
    def test_modifying_metadata_atomically(self):
        ...
        r = self.api.queries.execute_genquery(
            "select USER_NAME where META_USER_ATTR_NAME = 'a1' and META_USER_ATTR_VALUE = 'v1' and META_USER_ATTR_UNITS = 'u1'"
        )
        self.assertEqual(r["status_code"], 200)
...
class zoneTests(unittest.TestCase):
    ...
    def test_report_operation(self):
        ...
        self.assertIn(
            "schema_version", zone_report["zones"][0]["servers"][0]["server_config"]
        )
